...
def compute_offset(left_fit, right_fit, image_shape):
    ...
    offset = center_x - image.shape[1]/2
    
    print(offset*xm_per_pix, 'm')
...
for fname in glob.glob('test_images/*.jpg'):
    ...
    features = combined_thresh(undistorted)
    warped = warp(features, M)
    left_fit, right_fit = find_lanes(warped)
    update_lanes(warped, left_fit, right_fit)
    compute_curvature(left_fit, right_fit, image.shape[0])
    compute_offset(left_fit, right_fit, image.shape)
